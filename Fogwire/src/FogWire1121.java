		...
		setContentPane(mainPanel);
		{
...

			{
				...
				}
				{
					...
					StyleSheet ss = kit.getStyleSheet();
	                                ss.addRule("body {color:#0A246A; border:1px; padding:4px; font-size:10px; margin: 1px; }");
	                                ss.addRule("h1 {color: #0A246A; font-size:12px}");
	                                ss.addRule("h2 {color: #0A246A;}");
	...
	class FogWireBtnAction implements ActionListener {
		...
		public void btnAction (ActionEvent action)
		{
				...
				else
				{
					if(!(encChooser.isVisible()))
					{
						...
						try {
							...
							FogWire1121.fogWireSetDetails("File Successfully locked to C:\\FogWire2\\Locked_Docs\\locked.txt ",FogWire1121.encDetailArea); 
						} catch (FileNotFoundException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
			...
			else if (action.getSource() == btnLockNSend)
			{
				...
				if (FogWireLogin.checkEmail(toField.getText()) ) //&& FogWireLogin.checkRecipient(toField.getText()))
				{
...
//					    if(lastChar == 'y'){
//						s = new String(buffer, 0, size-1, "ISO8859_1");
//					}
	...
	public static void fogWireMessage (String message) throws FileNotFoundException
	{
		...
		JDialog dialog = optionPane.createDialog(null, "Status");
